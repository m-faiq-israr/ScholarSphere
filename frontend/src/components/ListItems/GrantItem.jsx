...
const GrantItem = ({ grant }) => {
  ...
  return (
    <div className="flex justify-between font-outfit p-1 h-full">
      <div className="w-[70%]">
        <h1 className="font-bold text-heading-1 pb-2">{grant.title}</h1>
        <p className="text-gray-600 line-clamp-2">{grant.description}</p>
      </div>

      <div className="w-[30%] pl-4 h-full flex flex-col justify-end items-end">
        ...
        <button
          className="flex select-none items-center justify-center mt-6 text-sm w-28 bg-green-500 text-white px-1 py-1 rounded-md hover:bg-green-600 transition-colors"
